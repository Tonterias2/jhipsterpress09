...
public class CactivityResource {
    ...
    */
    ...
    /**
     * SEARCH  /_search/cactivities?query=:query : search for the cactivity corresponding
     * to the query.
     *
     * @param query the query of the cactivity search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/cactivities")
    @Timed
    public ResponseEntity<List<CactivityDTO>> searchCactivities(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Cactivities for query {}", query);
        Page<CactivityDTO> page = cactivityService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/cactivities");
