...
public class CinterestResource {
    ...
    */
    ...
    /**
     * SEARCH  /_search/cinterests?query=:query : search for the cinterest corresponding
     * to the query.
     *
     * @param query the query of the cinterest search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/cinterests")
    @Timed
    public ResponseEntity<List<CinterestDTO>> searchCinterests(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Cinterests for query {}", query);
        Page<CinterestDTO> page = cinterestService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/cinterests");
