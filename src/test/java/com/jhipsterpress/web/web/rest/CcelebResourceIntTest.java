...
import com.jhipsterpress.web.repository.search.CcelebSearchRepository;
...
public class CcelebResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.CcelebSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    public void searchCceleb() throws Exception {
        // Initialize the database
        ccelebRepository.saveAndFlush(cceleb);
        when(mockCcelebSearchRepository.search(queryStringQuery("id:" + cceleb.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(cceleb), PageRequest.of(0, 1), 1));
        // Search the cceleb
        restCcelebMockMvc.perform(get("/api/_search/ccelebs?query=id:" + cceleb.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cceleb.getId().intValue())))
