...
public class CactivityResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.CactivitySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    public void searchCactivity() throws Exception {
        // Initialize the database
        cactivityRepository.saveAndFlush(cactivity);
        when(mockCactivitySearchRepository.search(queryStringQuery("id:" + cactivity.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(cactivity), PageRequest.of(0, 1), 1));
        // Search the cactivity
        restCactivityMockMvc.perform(get("/api/_search/cactivities?query=id:" + cactivity.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cactivity.getId().intValue())))
