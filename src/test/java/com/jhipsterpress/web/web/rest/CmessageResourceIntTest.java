...
public class CmessageResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.CmessageSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCmessage() throws Exception {
        ...
        // Validate the Cmessage in Elasticsearch
        verify(mockCmessageSearchRepository, times(1)).save(testCmessage);
    }
    ...
    public void createCmessageWithExistingId() throws Exception {
        ...
        // Validate the Cmessage in Elasticsearch
        verify(mockCmessageSearchRepository, times(0)).save(cmessage);
    }
    ...
    public void deleteCmessage() throws Exception {
        ...
        // Validate the Cmessage in Elasticsearch
        verify(mockCmessageSearchRepository, times(1)).deleteById(cmessage.getId());
    }

    @Test
    @Transactional
    public void searchCmessage() throws Exception {
        // Initialize the database
        cmessageRepository.saveAndFlush(cmessage);
        when(mockCmessageSearchRepository.search(queryStringQuery("id:" + cmessage.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(cmessage), PageRequest.of(0, 1), 1));
        // Search the cmessage
        restCmessageMockMvc.perform(get("/api/_search/cmessages?query=id:" + cmessage.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cmessage.getId().intValue())))
            .andExpect(jsonPath("$.[*].creationDate").value(hasItem(DEFAULT_CREATION_DATE.toString())))
