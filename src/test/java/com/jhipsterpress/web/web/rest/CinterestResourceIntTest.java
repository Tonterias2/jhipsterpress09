...
public class CinterestResourceIntTest {
     ...
     *
     * @see com.jhipsterpress.web.repository.search.CinterestSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createCinterest() throws Exception {
        ...
        // Validate the Cinterest in Elasticsearch
        verify(mockCinterestSearchRepository, times(1)).save(testCinterest);
    }
    ...
    public void createCinterestWithExistingId() throws Exception {
        ...
        // Validate the Cinterest in Elasticsearch
        verify(mockCinterestSearchRepository, times(0)).save(cinterest);
    }
    ...
    public void deleteCinterest() throws Exception {
        ...
        // Validate the Cinterest in Elasticsearch
        verify(mockCinterestSearchRepository, times(1)).deleteById(cinterest.getId());
    }

    @Test
    @Transactional
    public void searchCinterest() throws Exception {
        // Initialize the database
        cinterestRepository.saveAndFlush(cinterest);
        when(mockCinterestSearchRepository.search(queryStringQuery("id:" + cinterest.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(cinterest), PageRequest.of(0, 1), 1));
        // Search the cinterest
        restCinterestMockMvc.perform(get("/api/_search/cinterests?query=id:" + cinterest.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cinterest.getId().intValue())))
